 ...
 * 
 * @section chg_080228 2008-02-28 - v0.7
 * @todo Zaktualizowaæ !
 * 
 ...
 *   - Nowy komunikat akcji - #ACTN_SETCNT wysy³any do akcji ze stylem #ACTR_SETCNT w momencie, gdy
 *     zmieniany jest kontakt przypisany do kontrolki i trzeba np. uaktualniæ uchwyty.
 ...
 *   - Nowy typ powiadomienia: #ACTN_DEFAULT
 ...
 *       ...
 *       case IM_UIACTION:
 *         return ActionProc((sUIActionNotify*) msg->p1);
 *   @endcode
 *
 *   @change_to
 *   @code
 ...
 *     switch (msgBase->id) {
 *       case IM_PLUG_SDKVERSION:
 *         return KONNEKT_SDK_V; // Ta linijka MUSI zostaæ dodana!
 *       ...
 *       case IM_UIACTION:
 *         return ActionProc((sUIActionNotify_base*) msg->p1);
 *   @endcode
 *   @change_end
 * 
 ...
 *   @change_from
 *   @code
 *   int ActionProc(sUIActionNotify * an) {
 *     if ((an->act.id & IMIB_) == IMIB_CFG) {
 *       return ActionCfgProc(an);
 *     }
 *     switch (an->act.id) {
 *   @endcode
 *
 ...
 *     if ((anBase->act.id & IMIB_) == IMIB_CFG) {
 *       return ActionCfgProc(anBase);
 *     }
