 ...
 * @page dev_warn WA¯NE! ...
 *
 * Kilka BARDZO istotnych uwag. To w zasadzie swego rodzaju lista rzeczy do zrobienia w API...
 ...
 *   komunikatów (czy s¹ != 0, czy identyfikatory np. kontaktu czy akcji istniej¹ itp.)
 ...
 * oznaczany dalej jako @e NET. Jako sieæ rozumiemy grupê wtyczek jak¹ nasz dll mo¿e utworzyæ
 * z innymi dll'ami o tej samej wartoœci @e NET. Np. jedna wtyczka wysy³a emaile, 
 ...
 * wykonane. Jak najlepiej ustaliæ wartoœæ NET przeczytasz dalej, w sekcji @ref dev_guidelines.
 *
 ...
 * g³ównej - @a IMessageProc i kilka deklaracji z plików nag³ówkowych.
 * Jednak do pe³nego wykorzystania API, potrzebna bêdzie równie¿ reszta akcesoriów.
 * W plug_defs.cpp znajduj¹ siê definicje funkcji, które uproszcz¹ korzystanie
 * z API. Wiêkszoœæ funkcji korzysta ze zmiennej globalnej Ctrl (typu cCtrl) która
 ...
 * - do wszystkich wtyczek, podaj¹c jako NET wartoœæ #Net::broadcast.
 ...
 *  - Nowe identyfikatory typów wiadomoœci (@ref MT_) i statusów (@ref ST_) na potrzeby nowych wtyczek
 ...
 *  - Wszystkie identyfikatory w gotowych wtyczkach powinny byæ zg³oszone do SDK. Najlepiej w pliku nag³ówkowym...
 *  - Wszystkie tablice znaków (char*) zwracane przez dowolne funkcje rdzenia NIE MOG¥ byæ edytowane. 
 *    Zwracane wartoœci s¹ najczêœciej @b tymczasowe i kolejne wywo³anie funkcji API mo¿e bufor tymczasowy nadpisaæ...
 *    Je¿eli ich zawartoœæ bêdzie potrzebna po wywo³aniu jakiejkolwiek nastêpnej f-cji API musi ona zostaæ najpierw skopiowana.
 ...
 *  - U¿ytkownik programu to kontakt o pozycji 0. ¯eby sprawdziæ pozycjê maj¹c identyfikator u¿ywamy cCtrl::DTgetPos(DTCNT, id_kontaktu)
 ...
 *  - Czekam na WSZYSTKIE propozycje zmian. Je¿eli potrzebujecie u¿yæ nowego typu wiadomoœci, nowego identyfikatora sieci, 
 *    nowego typu ikonki czy nowego komunikatu - zapraszam do powiadomienia mnie o tym :) (@ref team) 
 ...
 *    Obie nie bêd¹ zg³aszaæ b³êdów wygenerowanych podczas prac developerskich. Obie do pobrania na stronie domowej SDK!
 *  - Informacje o wtyczce powinny byæ zapisane w zasobach (resources) DLLa. Jako IM_PLUG_VERSION, jeœli nie
 ...
 *    w przysz³oœci, ka¿dy komunikat bêdzie mia³ swoj¹ prototypow¹ funkcjê i ju¿ nie trzeba bêdzie tak uwa¿aæ :) ...
 ...
 *    identyfikatorem. Twoja pomy³ka mo¿e kosztowaæ potencjalnego u¿ytkownika sporo nerwów i byæ bardzo trudn¹ do wykrycia!
 ...
 *    Nazwa pliku powinna brzmieæ tak samo jak nazwa wtyczki i nie kolidowaæ z istniej¹cymi ju¿ nazwami...     
 *
 ...
 *
 * API Konnekta z za³o¿enia wykorzystuje jêzyk C++, ale tworzone jest z myœl¹ o 
 * mo¿liwoœci przygotowania kodu dzia³aj¹cego pod innym kompilatorem i w innym jêzyku. 
 ...
 *
 * Aktualnie %Konnekt i nasze wtyczki kompilowane s¹ w Visual C++ 2005,
 * z opcj¹ Multi-threaded DLL (opcje projektu / C++ / Code generation -> Runtime library), wiêc
