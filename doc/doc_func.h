...
G³ówna funkcja przetwarzaj¹ca komunikaty (wys³ane przez cCtrl::IMessage() ).
...
\warning IMessageProc \i MUSI obs³ugiwaæ cCtrl::error i komunikat #IM_PLUG_INIT
   ...
   int __stdcall IMessageProc(unsigned int id , int p1 , int p2 , unsigned int sender) {
     switch (id) {
        case IM_PLUG_INIT:        // Inicjalizacja wtyczki.
           Ctrl=(cCtrl*)p1;       // Przyjêcie adresu cCtrl
