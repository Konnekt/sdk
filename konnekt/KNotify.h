/*
  nazwa="K.Notify"
  info="Nag³ówek wtyczki K.Notify."
...
// API zewnêtrzne
  ...
  namespace IM {
    const int show = 774201; /**<  Wyœwietla informacjê nad zegarkiem systemowym. 
                                   \return (int) identyfikator u¿ywany do usuwania wiadomoœci.
                                   @sa KNotify::sIMessage_notify  */
    const int hide = 774202; /**<  Ukrywa informacjê  wed³ug numeru identyfikacyjnego zwracanego podczas wstawiania wiadomoœci
  ...
  */
  ...
  public:
    ...
    sIMessage_notify(const char * text, unsigned int icoID, const unsigned char msgType = tInform, unsigned char timeToLive = 1)
      :sIMessage_base(IM::show, KNotify::net, -1) 
    ...
    sIMessage_notify(sIMessage_notify * msg)
      :sIMessage_base(msg) 
    {
      s_size=sizeof(sIMessage_notify);
      if (msg->s_size == sizeof(sIMessage_notify_v1)) { 
        ...
        this->_icoID = msg1->mask == 0 ? (int)msg1->ico : msg1->mask == 1 ? UIIcon(IT_STATUS, msg1->cntnet, msg1->status, 0) : msg1->mask == 2 ? UIIcon(IT_MESSAGE, 0, MT_MESSAGE, 0) : 0 ;
  ...
  */
  ...
  public:
    ...
    sIMessage_notify_kinfo(const char * text, unsigned int icoID, const unsigned char msgType = tInform, unsigned char timeToLive = 1)
      :sIMessage_notify(text, icoID, msgType, timeToLive)
      , _infoType(0), _infoArgs(0)
    {
    ...
    sIMessage_notify_kinfo(const char * text, HICON ico, const unsigned char msgType = tInform, unsigned char timeToLive = 1)
      :sIMessage_notify(text, ico, msgType, timeToLive)
      , _infoType(0), _infoArgs(0)
    {
      s_size=sizeof(sIMessage_notify_kinfo);
    }
